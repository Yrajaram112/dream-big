...
const DepositForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus(data.error || "Failed to submit data.");
      } else {
        setStatus("Deposit data submitted successfully!");
        setFormData({
          name: "",
          amount: "",
          remarks: "",
          ...
          <div className="w-full px-4 lg:w-7/12 xl:w-8/12">
            <div className="wow fadeInUp shadow-three dark:bg-gray-dark mb-12 rounded-sm bg-white px-8 py-11 sm:p-[55px] lg:mb-5 lg:px-8 xl:p-[55px]" data-wow-delay=".15s">
              <div className="mb-4 flex items-center justify-between">
                ...
                <a
                  href="/admin"
                  className="rounded-sm bg-primary px-6 py-2 text-base font-medium text-white"
                >
                  Admin
                </a>
              </div>
              ...
              <p className="mb-11 border-b border-body-color border-opacity-25 pb-11 text-base leading-relaxed text-body-color dark:border-white dark:border-opacity-25">
                All types of Deposit/Credit for Office
              </p>
              <form onSubmit={handleSubmit}>
                <select
                  ...
                  className="mb-4 w-full rounded-sm border bg-[#f8f8f8] px-6 py-3 text-base text-body-color outline-none focus:border-primary dark:border-transparent dark:bg-[#2C303B] dark:focus:border-primary dark:focus:shadow-none"
                >
                  <option value="">Deposited by</option>
